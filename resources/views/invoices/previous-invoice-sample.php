...
<head>
    ...
    <style>
        ...
        .logo { height: 130px; margin-bottom: 30px; }
        .company-details { line-height: 1.35; }
        .label {
            font-weight: bold;
        ...
        .clear { clear: both; }
        h1 { margin: 0 0 15px 0; font-size: 30px; letter-spacing: 2px; }
...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="right">
            ...
            {{-- <h1>SOLICITOR INVOICE</h1> --}}
            <h1>{{ $documentType ?? 'SOLICITOR INVOICE' }}</h1>

            <table class="invoice-box">
                <tr>
                    <td class="label-cell">Invoice Number</td>
                    <td class="value-cell">SOL-INV#{{ $invoice->id }}</td>
                </tr>
                <tr>
...
<strong>{{ $invoice->client?->client_name ?? '-' }}</strong>
<div style="margin-top: 10px;">
        <div class="label">Solicitor</div>
        <strong>{{ $invoice->solicitor?->name ?? '-' }}</strong>
    </div>
                <div>
                    <span class="status-box {{ $invoice->payment_status === 'paid' ? 'paid' : 'unpaid' }}">
...
@php
    ...
    // Calculate subtotal directly from the individual items
...
<table>
    ...
    <tbody>
        ...
        @empty
            <tr>
                <td>Legal / Solicitor Services</td>
...
<div class="summary">
    <table>
        ...
        <tr>
            <td>VAT ({{ number_format($vatRate, 2) }}%)</td>
...
<div class="clear"></div>
    ...
    <div class="footer-note">
        This is a computer generated solicitor invoice and does not require a signature.
    </div>
